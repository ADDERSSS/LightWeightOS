...
void show_mem_info (_boot_info_t * boot_info) {
...
int memory_create_map (pde_t * page_dir, uint32_t vaddr, uint32_t paddr, int count, uint32_t prem) {
    for (int i = 0; i < count; i ++) {
        //log_printf("create map: v-0x%x, p-0x%x, prem:0x%x", vaddr, paddr, prem);
...
void create_kernel_table (void) {
    ...
    static memory_map_t kernel_map[] = {
        ...
        {s_data, (void *)MEM_EDBA_START, s_data, PTE_W},
...
static uint32_t total_mem_size (_boot_info_t * boot_info) {
    uint32_t mem_size = 0;
    for (int i = 0; i < boot_info->ram_region_count; i ++) {
...
void memory_init (_boot_info_t * boot_info) {
    ...
    ASSERT(mem_free < (uint8_t *)MEM_EDBA_START);
